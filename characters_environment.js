...
function play(){
  gameStatus="Start";
  document.getElementById('status').innerHTML.textContent="The game is loading";
}
function reStart(){
...
function game(){
  ...
  if(gameConfig.status==='gameover'){
    ...
    textSize(15);
    text("Press Restart Button to Restart", gameConfig.screenX/2, gameConfig.screenY/2+135);
    textSize(40);
...
function changeGameStatud(character){
  console.log("Inside revive parent");
  if(gameStatus=="Start" && noseX!="" && gameConfig.status==="start") {
    world_start.play();
    initializeCharacterStatus(mario);
    gameConfig.status= "play";
  }
  if(gameConfig.status==="gameover" && Gameestatus=="start"){
    console.log("Inside revive");
...
function jumping(character){
	if( (noseY<175 &&character.live) || (touchIsDown&&character.live) ){
    mario_jump.play();
